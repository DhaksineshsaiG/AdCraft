  ...
  return (
    ...
    >
      ...
      <div className="px-5 py-4 space-y-4">
        ...
        {/* Generated poster summary card */}
        {isDone && generatedPoster && (
          <div className="flex items-center gap-3.5 p-3 rounded-xl border border-emerald-100 dark:border-emerald-900/30 bg-emerald-50/40 dark:bg-emerald-950/20">
            {generatedPoster.posterUrl ? (
              <div className="relative h-16 w-16 shrink-0 rounded-lg overflow-hidden border border-emerald-200/60 dark:border-emerald-800/40 bg-white dark:bg-slate-800 shadow-xs">
                <img
                  src={generatedPoster.posterUrl}
                  alt={generatedPoster.title || 'Generated poster'}
                  className="h-full w-full object-cover"
                />
              </div>
            ) : (
              <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-lg bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600 dark:text-emerald-400">
                <ImageIcon className="h-6 w-6" />
              </div>
            )}
            <div className="min-w-0 flex-1">
              <h4 className="text-xs font-semibold text-slate-900 dark:text-slate-100 truncate">
                {generatedPoster.title || generatedPoster.productName || 'Generated Poster'}
              </h4>
              {generatedPoster.productName && generatedPoster.title !== generatedPoster.productName && (
                <p className="text-[11px] text-slate-500 dark:text-slate-400 truncate mt-0.5">
                  {generatedPoster.productName}
                </p>
              )}
              <div className="flex flex-wrap gap-1.5 mt-1.5">
                {generatedPoster.style && (
                  <span className="inline-flex items-center rounded-md px-1.5 py-0.5 text-[10px] font-medium bg-emerald-100/80 text-emerald-800 dark:bg-emerald-900/50 dark:text-emerald-300 capitalize">
                    {generatedPoster.style}
                  </span>
                )}
                {generatedPoster.size && (
                  <span className="inline-flex items-center rounded-md px-1.5 py-0.5 text-[10px] font-medium bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300 capitalize">
                    {generatedPoster.size}
                  </span>
                )}
                {generatedPoster.format && (
                  <span className="inline-flex items-center rounded-md px-1.5 py-0.5 text-[10px] font-medium bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300 uppercase">
                    {generatedPoster.format}
                  </span>
                )}
              </div>
            ...
            {isDone && (
              <>
                ...
                {onViewPoster ? (
                  ...
                  <button type="button" onClick={onDismiss} className="btn btn-secondary btn-sm flex-1">
                    View poster
                  </button>
                ) : null}
              </>
